...
static inline int16_t reg_int_cb(struct int_param_s *int_param)
{
...
#include "../global.h"
#include <libpic30.h>
extern sys_timers_t g_tmr;

#define i2c_write I2Cdev_writeBytes
#define i2c_read I2Cdev_readBytes
#define delay_ms(d) \
  { __delay32( (uint32_t) (((int64_t) d)*(FCY)/1000ULL)); }

int16_t get_ms(uint32_t *count)
{
	if (!count)
        return 1;
    count[0] = g_tmr.time_stamp;//?????
    return 0;
}
...
#define log_i(...)     do {} while (0)
#define log_e(...)     do {} while (0)
#define reg_int_cb(d)		do {} while (0)
#else
...
static int16_t set_int_enable(uint8_t enable);
...
struct chip_cfg_s {
    ...
    uint8_t fifo_enable;
    /* Matches int16_t enable register. */
    uint8_t int_enable;
...
#ifdef AK89xx_SECONDARY
static int16_t setup_compass(void);
...
int16_t mpu_read_6500_gyro_bias(int32_t *gyro_bias) {
	uint8_t data[6];
...
#ifdef MPU6050
static int16_t get_accel_prod_shift(float *st_shift)
{
...
static int16_t accel_self_test(int32_t *bias_regular, int32_t *bias_st)
{
    int16_t jj, result = 0;
...
static int16_t gyro_self_test(int32_t *bias_regular, int32_t *bias_st)
{
    int16_t jj, result = 0;
    uint8_t tmp[3];
...
#endif 
#ifdef AK89xx_SECONDARY
static int16_t compass_self_test(void)
{
    uint8_t tmp[6];
    uint8_t tries = 10;
    int16_t result = 0x07;
...
static int16_t get_st_biases(int32_t *gyro, int32_t *accel, uint8_t hw_test)
{
    uint8_t data[MAX_PACKET_LENGTH];
...
static int16_t accel_6500_self_test(int32_t *bias_regular, int32_t *bias_st, int16_t debug)
{
    int16_t i, result = 0, otp_value_zero = 0;
...
static int16_t gyro_6500_self_test(int32_t *bias_regular, int32_t *bias_st, int16_t debug)
{
    int16_t i, result = 0, otp_value_zero = 0;
...
static int16_t get_st_6500_biases(int32_t *gyro, int32_t *accel, uint8_t hw_test, int16_t debug)
{
    uint8_t data[HWST_MAX_PACKET_LENGTH];
    uint8_t packet_count, ii;
    uint16_t fifo_count;
    int16_t s = 0, read_size = 0, ind;
    ...
    gyro[0] = (int32_t)(((int64_t)gyro[0]<<16) / test.gyro_sens / s);
    gyro[1] = (int32_t)(((int64_t)gyro[1]<<16) / test.gyro_sens / s);
    gyro[2] = (int32_t)(((int64_t)gyro[2]<<16) / test.gyro_sens / s);
    accel[0] = (int32_t)(((int64_t)accel[0]<<16) / test.accel_sens / s);
    accel[1] = (int32_t)(((int64_t)accel[1]<<16) / test.accel_sens / s);
    accel[2] = (int32_t)(((int64_t)accel[2]<<16) / test.accel_sens / s);
    ...
    uint16_t lpa_freq)
{
    ...
    if (lpa_freq) {
        ...
        uint16_t ii;
        int8_t *cache_ptr = (int8_t*)&st.chip_cfg.cache;
